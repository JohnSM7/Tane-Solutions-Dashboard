...
import {
  ...
  facturarContrato, necesitaFacturacion, periodoLabel, importeFacturacion,
...
const { contratos, loading, pendientesFacturacion } = useContratos();

// ── Facturación recurrente ────────────────────────────────────────────────────
const facturando     = ref<Set<string>>(new Set());
const facturadoOk    = ref<Set<string>>(new Set());

const facturarUno = async (c: Contrato) => {
  facturando.value.add(c.id);
  try {
    await facturarContrato(c);
    c.ultima_facturacion = new Date().toISOString().split('T')[0] ?? null;
    facturadoOk.value.add(c.id);
    setTimeout(() => facturadoOk.value.delete(c.id), 3000);
  } catch (e: any) {
    alert('Error al generar factura: ' + (e.message ?? 'Error desconocido'));
  } finally {
    facturando.value.delete(c.id);
  }
};

const facturarTodos = async () => {
  const pendientes = pendientesFacturacion.value;
  if (pendientes.length === 0) return;
  if (!confirm(`¿Generar ${pendientes.length} factura(s) recurrente(s) del período actual?`)) return;
  for (const c of pendientes) await facturarUno(c);
};
...
<template>
  <div class="view-container">
    ...
    <!-- Panel facturación recurrente -->
    <DashboardCard v-if="!loading && pendientesFacturacion.length > 0">
      <template #actions>
        <button class="btn-facturar-all" @click="facturarTodos">
          ⚡ Generar todas ({{ pendientesFacturacion.length }})
        </button>
      </template>
      <div class="recurrentes-header">
        <span class="recurrentes-title">🔁 Facturación recurrente pendiente</span>
        <span class="recurrentes-sub">{{ pendientesFacturacion.length }} servicio(s) sin facturar este período</span>
      </div>
      <div class="recurrentes-list">
        <div v-for="c in pendientesFacturacion" :key="c.id" class="recurrente-row">
          <div class="recurrente-info">
            <span class="recurrente-nombre">{{ c.nombre }}</span>
            <span class="recurrente-cliente muted">{{ c.clientes?.nombre ?? '—' }}</span>
            <span class="recurrente-concepto muted small">{{ periodoLabel(c) }}</span>
          </div>
          <div class="recurrente-right">
            <span class="tipo-badge">{{ c.tipo }}</span>
            <span class="recurrente-importe">{{ fmt(importeFacturacion(c)) }}</span>
            <button
              class="btn-facturar"
              :class="{ ok: facturadoOk.has(c.id) }"
              :disabled="facturando.has(c.id) || facturadoOk.has(c.id)"
              @click="facturarUno(c)"
            >
              {{ facturadoOk.has(c.id) ? '✓ Facturado' : facturando.has(c.id) ? '...' : 'Facturar' }}
            </button>
          </div>
        </div>
      </div>
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr v-for="c in filtered" :key="c.id">
              ...
              <td data-label="Últ. factura">
                <span v-if="necesitaFacturacion(c)" class="badge-pendiente">Pendiente</span>
                <span v-else-if="c.ultima_facturacion" class="muted small">
                  {{ new Date(c.ultima_facturacion).toLocaleDateString('es-ES') }}
                </span>
              ...
              <td data-label="Acciones">
                <div class="row-actions">
                  <button
                    v-if="necesitaFacturacion(c)"
                    class="btn-facturar-sm"
                    :disabled="facturando.has(c.id)"
                    @click="facturarUno(c)"
                    title="Generar factura de este período"
                  >⚡ Facturar</button>
...
/* ── Facturación recurrente ─────────────────────────────────────────────────── */
.recurrentes-header { margin-bottom: 1rem; }
.recurrentes-title  { font-weight: 700; font-size: 1rem; display: block; margin-bottom: 0.2rem; }
.recurrentes-sub    { font-size: 0.82rem; color: var(--color-text-muted); }

.recurrentes-list { display: flex; flex-direction: column; gap: 0.6rem; }
.recurrente-row {
  display: flex; align-items: center; justify-content: space-between; gap: 1rem;
  padding: 0.85rem 1rem; background: rgba(227,255,4,0.04);
  border: 1px solid rgba(227,255,4,0.15); border-radius: 8px; flex-wrap: wrap;
}
.recurrente-info { display: flex; flex-direction: column; gap: 0.15rem; flex: 1; min-width: 0; }
.recurrente-nombre  { font-weight: 600; font-size: 0.95rem; }
.recurrente-cliente { font-size: 0.82rem; }
.recurrente-concepto { font-size: 0.78rem; }
.recurrente-right { display: flex; align-items: center; gap: 0.75rem; flex-shrink: 0; flex-wrap: wrap; }
.recurrente-importe { font-weight: 800; font-size: 1.05rem; color: var(--color-primary); white-space: nowrap; }

.btn-facturar {
  background: var(--color-primary); color: #000; font-weight: 700;
  border: none; border-radius: 6px; padding: 0.45rem 1rem; cursor: pointer;
  font-size: 0.88rem; white-space: nowrap; transition: opacity 0.2s;
}
.btn-facturar:disabled { opacity: 0.6; cursor: not-allowed; }
.btn-facturar.ok { background: #4ade80; cursor: default; }

.btn-facturar-all {
  background: var(--color-primary); color: #000; font-weight: 700;
  border: none; border-radius: 6px; padding: 0.4rem 0.9rem; cursor: pointer;
  font-size: 0.85rem; white-space: nowrap;
}
.btn-facturar-all:hover { opacity: 0.85; }

.btn-facturar-sm {
  background: rgba(227,255,4,0.12); color: var(--color-primary);
  border: 1px solid rgba(227,255,4,0.35); font-size: 0.8rem; font-weight: 700;
  padding: 0.25rem 0.6rem; border-radius: 4px; cursor: pointer; white-space: nowrap;
}
.btn-facturar-sm:hover { background: rgba(227,255,4,0.22); }
.btn-facturar-sm:disabled { opacity: 0.5; cursor: not-allowed; }

.badge-pendiente {
  background: rgba(255,165,0,0.15); color: #ffa500;
  border: 1px solid rgba(255,165,0,0.3); font-size: 0.72rem; font-weight: 700;
